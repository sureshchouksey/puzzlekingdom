...
| AI | Pluggable provider (`AI_PROVIDER` env var): Anthropic Claude (default) or Google Gemini, both called server-side with the uploaded PDF/image as a native document/image input and a schema-constrained call that forces the response into a strict question shape — never free text parsed after the fact. |
