...
- **Never interpolate a bare JS `Date` into a raw Drizzle `sql\`\`` template** — unlike Drizzle's query builder, the `sql` tagged-template helper does not auto-convert a `Date`; the driver binds it via `Date.prototype.toString()` (e.g. `"Thu Aug 06 2026 22:56:53 GMT+0100 (British Summer Time)"`), which Postgres can't parse as a timestamp, so every query fails with a 500. Real bug, found 5 September 2026 in `tutorInsights.ts`'s `getDoubtBreakdown` (a 30-day lookback cutoff). Fix: call `.toISOString()` on the `Date` before interpolating it, or (preferred where possible, see `tutorBudget.ts`'s `checkDailyCap`) let Postgres compute the date itself (`date_trunc('day', now())`) instead of interpolating a JS value at all.
